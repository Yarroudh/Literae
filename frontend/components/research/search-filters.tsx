...
export function SearchFilters({ filters, onChange }: SearchFiltersProps) {
  return (
    <div className="grid gap-4 md:grid-cols-2">
      <FilterGroup eyebrow="Results" title="Retrieval">
      ...
      <FilterGroup eyebrow="Publication" title="Scope">
      ...
      <FilterGroup eyebrow="People & places" title="Research context" wide>
...
function FilterGroup({ eyebrow, title, wide = false, children }: { eyebrow: string; title: string; wide?: boolean; children: React.ReactNode }) {
  return <section className={`rounded-2xl border border-[var(--line)] bg-[var(--panel-2)] p-4 sm:p-5 ${wide ? "md:col-span-2" : ""}`}><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[var(--accent)]">{eyebrow}</p><h3 className="mt-1 text-sm font-bold text-[var(--ink)]">{title}</h3><div className={`mt-4 grid gap-3 ${wide ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}>{children}</div></section>;
}
